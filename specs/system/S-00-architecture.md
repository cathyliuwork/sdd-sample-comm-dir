...
**Purpose**: 社区成员名录项目的系统架构设计和技术栈定义，支持多社区成员信息收集和管理。
...
社区成员名录是一个多社区成员信息收集和管理的移动 Web 应用。系统允许管理员创建和管理多个社区，每个社区拥有独立的成员信息收集表单和展示页面。普通用户无需登录即可填写表单和查看成员列表。
...
C4Context
    ...
    System(wechat_intro, "社区成员名录系统", "多社区成员信息收集和管理平台")
